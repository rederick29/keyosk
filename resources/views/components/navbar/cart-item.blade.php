...
<div
    {{ $attributes->merge(['class' => 'w-full px-5 flex flex-row gap-5 items-center text-center rounded-lg bg-zinc-900 transition-colors duration-500']) }}>
        ...
        <form id="cart-{{ $productId }}" class="flex flex-row w-full justify-between items-center" method="POST"
            ...
            <div class="flex flex-row ring-violet-700 ring-2 rounded-md">
                <div id="decrease-{{ $productId }}"
                    class="size-7 flex items-center justify-center bg-zinc-700 rounded-bl-md rounded-tl-md">
                ...
                <input name="quantity_input" id="cart_quantity_input-{{ $productId }}"
                    class="cart-quantity w-10 h-7 px-[0.33rem] flex items-center justify-center bg-zinc-800 outline-none"
                    value="{{ $productQuantity }}">
                <div id="increase-{{ $productId }}"
                    class="size-7 flex items-center justify-center bg-zinc-700 rounded-br-md rounded-tr-md">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"
                        fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                        stroke-linejoin="round">
