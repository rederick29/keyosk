  ...
  <form action="{{ route('contact.send') }}" method="POST" class="mx-auto mt-10 max-w-lg lg:mt-10">
    ...
    <div class="grid grid-cols-1 gap-x-8 gap-y-5 lg:grid-cols-2">
      <div>
        ...
        <div class="mt-2.5">
          <input type="text" name="first-name" id="first-name" class="block font-semibold w-full rounded-lg  px-3.5 py-2 text-white bg-zinc-700" required>
        </div>
      </div>
      <div>
        <label for="last-name" class="block font-semibold text-lg/6 text-white text-sm">Last name</label>
        <div class="mt-2.5">
          <input type="text" name="last-name" id="last-name" class="block font-semibold w-full rounded-lg  px-3.5 py-2 text-white bg-zinc-700" required>
        </div>
      </div>
      <div class="lg:col-span-2">
        <label for="email" class="block font-semibold text-lg/6 text-white text-sm">Email</label>
        <div class="mt-2.5">
          <input type="email" name="email" id="email" class="block font-semi bold w-full rounded-lg  px-3.5 py-2 text-white bg-zinc-700" required>
        </div>
      </div>
      <div class="lg:col-span-2">
        <label for="subject" class="block font-semibold  text-lg/6 text-white text-sm">Subject</label>
        <div class="mt-2.5">
          <input type="text" name="subject" id="subject" class="block font-semibold w-full rounded-lg  px-3.5 py-2 text-white bg-zinc-700">
        </div>
      </div>
      <div class="lg:col-span-2">
        <label for="message" class="block font-semibold text-lg/6 text-white text-sm">Message</label>
        <div class="mt-2.5">
          <textarea name="message" id="message" rows="4" class="block font-semibold w-full rounded-lg  px-3.5 py-2 text-white placeholder:text-white-400 bg-zinc-700" minlength="10" maxlength="250" required></textarea>
        </div>
      </div>
    </div>
    <div class="mt-10">
      <button type="submit" name="submit" class="block font-semi bold w-full rounded-lg bg-violet-700 px-3.5 py-2.5 text-center text-base text-white hover:bg-violet-500" onSubmit="msgSent()">Submit</button>
    </div>
  </form>
